...
# measurement model for the 8 factor solution 
...
# fit_mod_efa8
efa8_out <- lavaan::cfa(efa8_mod, data = cfa_data, estimator = "WLSMV", ordered = names(cfa_data), std.lv = TRUE)

efa8_sum <- lavaan::summary(efa8_out, fit.measure = TRUE, standardized = TRUE, rsquare = TRUE)

# mod indices
efa8_mi <- lavaan::modindices(efa8_out, sort = TRUE, minimum.value = 3.84) 
efa8_mi

# std resids
efa8_stdresid <- lavaan::lavResiduals(efa8_out, type = "raw")
efa8_stdresid

# collapse the stats and maths test anxiety latents because of the non-pos-def var-cov matrix
...
maths_stats_test =~ rmars_test10 + rmars_test9 + rmars_test2 + stars_m_test8 + rmars_s_test9 + rmars_s_test2 + rmars_s_test10
'

# fit_mod_efa8_npd
efa8_npd_out <- lavaan::cfa(efa8_npd_mod, data = cfa_data, estimator = "WLSMV", ordered = names(cfa_data), std.lv = TRUE)

efa8_npd_sum <- lavaan::summary(efa8_npd_out, fit.measure = TRUE, standardized = TRUE, rsquare = TRUE)

# mod indices
efa8_npd_mi <- lavaan::modindices(efa8_npd_out, sort = TRUE, minimum.value = 3.84) 
efa8_npd_mi

# std resids
efa8_npd_stdresid <- lavaan::lavResiduals(efa8_npd_out, type = "raw")
efa8_npd_stdresid
...
'
# fit mods one & four
one_factor_out <- lavaan::cfa(one_factor_mod, data = cfa_data, estimator = "WLSMV", ordered = names(cfa_data), std.lv = TRUE)

one_factor_sum <- lavaan::summary(one_factor_out, fit.measure = TRUE, standardized = TRUE, rsquare = TRUE)

four_factor_out <- lavaan::cfa(four_factor_mod, data = cfa_data, estimator = "WLSMV", ordered = names(cfa_data), std.lv = TRUE)

four_factor_sum <- lavaan::summary(four_factor_out, fit.measure = TRUE, standardized = TRUE, rsquare = TRUE)

one_four_compare <- lavaan::anova(one_factor_out, four_factor_out) # this is gonna' be sig anyway!

one_factor_sum
four_factor_sum

# mod indices
one_factor_mi <- lavaan::modindices(one_factor_out, sort = TRUE) 
one_factor_mi

four_factor_mi <- lavaan::modindices(four_factor_out, sort = TRUE) 
four_factor_mi
...
# fit_mod_bifactor
bifactor_out <- lavaan::cfa(bifactor_mod, data = cfa_data, estimator = "WLSMV", ordered = names(cfa_data), std.lv = TRUE, orthogonal = TRUE)

bifactor_sum <- lavaan::summary(bifactor_out, fit.measure = TRUE, standardized = TRUE, rsquare = TRUE)
bifactor_sum

bifactor_mi <- lavaan::modindices(bifactor_out, sort = TRUE)

bifactor_table <- lavaan::parameterTable(bifactor_out) |>
...
fit_subset <- c("chisq.scaled", "df.scaled", "pvalue.scaled", "cfi.scaled", "tli.scaled", "rmsea.scaled", "rmsea.ci.upper.scaled", "rmsea.ci.lower.scaled", "srmr")

#null RMSEAs
efa8_npd_nullRMSEA <- semTools::nullRMSEA(efa8_npd_out)
one_factor_nullRMSEA <- semTools::nullRMSEA(one_factor_out)
four_factor_nullRMSEA <- semTools::nullRMSEA(four_factor_out)
bifactor_nullRMSEA <- semTools::nullRMSEA(bifactor_out)

null_RMSEA <- c(efa8_npd_nullRMSEA
                , one_factor_nullRMSEA
                , four_factor_nullRMSEA
                , bifactor_nullRMSEA
                )

cfa_fit_summary <- round(rbind(lavaan::fitmeasures(efa8_npd_out, fit_subset)
                           , lavaan::fitmeasures(one_factor_out, fit_subset)
                           , lavaan::fitmeasures(four_factor_out, fit_subset)
                           , lavaan::fitmeasures(bifactor_out, fit_subset)
                     ), 3)


fit_summary <- round(cbind(cfa_fit_summary
                           , null_RMSEA), 3)

fit_summary <- tibble::as_tibble(fit_summary)

fit_summary <- fit_summary |> 
  dplyr::relocate(null_RMSEA, .after = tli.scaled)
...
fit_summary <- cbind(`CFA Model`
                     , fit_summary
                     )

colnames(fit_summary) <- c("CFA Model"
                           , "Chi-Sq "
                           , "Degrees of Freedom"
                           , "Chi-Sq p-value" 
                           , "CFI" 
