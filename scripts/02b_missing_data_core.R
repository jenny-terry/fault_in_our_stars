...
core_item_missing <- core_data %>% 
  dplyr::select(c(Q7.1_1:Q17.3)) %>%
  finalfit::ff_glimpse()

core_item_missing_max <- max(core_item_missing$Continuous$missing_percent)

# check % item missing per scale
core_stars_item_missing <- core_data %>%
  dplyr::select(c(Q7.1_1:Q7.1_24)) %>%
  naniar::pct_miss()

core_rmars_s_item_missing <- core_data %>%
  dplyr::select(c(Q7.2_1:Q7.2_19, Q8.2_11)) %>%
  naniar::pct_miss()

core_stars_m_item_missing <- core_data %>%
  dplyr::select(c(Q8.1_1:Q8.1_20, Q7.1_9, Q7.1_12, Q7.1_17)) %>%
  naniar::pct_miss()

core_rmars_item_missing <- core_data %>%
  dplyr::select(c(Q8.2_1:Q8.2_20)) %>%
  naniar::pct_miss()

core_trait_item_missing <- core_data %>%
  dplyr::select(c(Q9.1_1:Q9.1_21)) %>%
  naniar::pct_miss()

core_test_item_missing <- core_data %>%
  dplyr::select(c(Q10.1_1:Q10.1_25)) %>%
  naniar::pct_miss()

core_fne_item_missing <- core_data %>%
  dplyr::select(c(Q11.1_1:Q11.1_8)) %>%
  naniar::pct_miss()

core_social_item_missing <- core_data %>%
  dplyr::select(c(Q12.1_1:Q12.1_24)) %>%
  naniar::pct_miss()

core_creativity_item_missing <- core_data %>%
  dplyr::select(c(Q13.1_1:Q13.1_8)) %>%
  naniar::pct_miss()

core_iou_item_missing <- core_data %>%
  dplyr::select(c(Q14.1_1:Q14.1_12)) %>%
  naniar::pct_miss()

core_efficacy_item_missing <- core_data %>%
  dplyr::select(c(Q15.1_1:Q15.1_8)) %>%
  naniar::pct_miss()

core_persist_item_missing <- core_data %>%
  dplyr::select(c(Q16.1_2:Q16.1_7)) %>%
  naniar::pct_miss()

core_crt_item_missing <- core_data %>%
  dplyr::select(c(Q17.1:Q17.3)) %>%
  naniar::pct_miss()

core_uni_grade_item_missing <- core_data %>%
  dplyr::select(c(uni_grade_num_z)) %>%
  naniar::pct_miss()

core_sr_grade_item_missing <- core_data %>%
  dplyr::select(c(sr_grade_num_z)) %>%
  naniar::pct_miss()

### COMPOSITE & SUBSCALE SCORING
...
# add composites to data

#readr::write_csv(core_data, here::here("data/core_data.csv"))

### COMPOSITE LEVEL MISSING

core_stars_comp_missing <- core_data %>%
  dplyr::select(stars) %>%
  naniar::pct_miss()

core_rmars_s_comp_missing <- core_data %>%
  dplyr::select(rmars_s) %>%
  naniar::pct_miss()

core_stars_m_comp_missing <- core_data %>%
  dplyr::select(stars_m) %>%
  naniar::pct_miss()

core_rmars_comp_missing <- core_data %>%
  dplyr::select(rmars) %>%
  naniar::pct_miss()

core_trait_comp_missing <- core_data %>%
  dplyr::select(trait_anx) %>%
  naniar::pct_miss()

core_test_comp_missing <- core_data %>%
  dplyr::select(test_anx) %>%
  naniar::pct_miss()

core_fne_comp_missing <- core_data %>%
  dplyr::select(fne) %>%
  naniar::pct_miss()

core_social_comp_missing <- core_data %>%
  dplyr::select(social_anx) %>%
  naniar::pct_miss()

core_creativity_comp_missing <- core_data %>%
  dplyr::select(creativity_anx) %>%
  naniar::pct_miss()

core_iou_comp_missing <- core_data %>%
  dplyr::select(iou) %>%
  naniar::pct_miss()

core_efficacy_comp_missing <- core_data %>%
  dplyr::select(self_efficacy) %>%
  naniar::pct_miss()

core_persist_comp_missing <- core_data %>%
  dplyr::select(persistence) %>%
  naniar::pct_miss()

core_crt_comp_missing <- core_data %>%
  dplyr::select(crt_n_correct) %>%
  naniar::pct_miss()

core_uni_grade_comp_missing <- core_data %>%
  dplyr::select(uni_grade_num_z) %>%
  naniar::pct_miss()

core_sr_grade_comp_missing <- core_data %>%
  dplyr::select(sr_grade_num_z) %>%
  naniar::pct_miss()

core_var_names <- c("STARS", "R-MARS-S", "STARS-M", "R-MARS", 
...
core_percent_item_missing <- c(core_stars_item_missing, core_rmars_s_item_missing, core_stars_m_item_missing, core_rmars_item_missing,
                               core_trait_item_missing, core_test_item_missing, core_fne_item_missing, core_social_item_missing,
                               core_creativity_item_missing, core_iou_item_missing, core_efficacy_item_missing, core_persist_item_missing,
                               core_crt_item_missing, core_uni_grade_item_missing, core_sr_grade_item_missing) |> round (3)
core_percent_comp_missing <- c(core_stars_comp_missing, core_rmars_s_comp_missing, core_stars_m_comp_missing, core_rmars_comp_missing,
                               core_trait_comp_missing, core_test_comp_missing, core_fne_comp_missing, core_social_comp_missing,
                               core_creativity_comp_missing, core_iou_comp_missing, core_efficacy_comp_missing, core_persist_comp_missing,
                               core_crt_comp_missing, core_uni_grade_comp_missing, core_sr_grade_comp_missing) |> round (3)
core_missing_table <- tibble::tibble(core_var_names, core_percent_item_missing, core_percent_comp_missing) |> 
  dplyr::arrange(abs(core_percent_item_missing))
